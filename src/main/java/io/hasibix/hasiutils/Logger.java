...
public class Logger {
	...
	void writeLog(String text, Type type) {
		...
		String line = String.format("[%s] [%s] [%s]: %s", Color.CYAN + dtf.format(now) + Color.RESET,
				getTypeString(true, type) + Color.RESET, this.clazz.getSimpleName(), text);
		String lineNoColor = String.format("[%s] [%s] [%s]: %s", dtf.format(now), getTypeString(false, type),
				this.clazz.getName(), text);
		...
		if (logsPath != null) {
			...
			try {
				...
				Files.writeString(Paths.get(logsPath + fdtf.format(now) + ".log"), lineNoColor + System.lineSeparator(),
						StandardOpenOption.CREATE, StandardOpenOption.APPEND);

			} catch (IOException e) {
				String er = String.format("[%s] [%s] [%s]: %s", Color.CYAN + dtf.format(now) + Color.RESET,
						Color.RED + "ERR" + Color.RESET, Logger.class.getName(),
						Color.RESET + "An exception occurred in logger.\n" + ExceptionUtils.getStackTrace(e));
